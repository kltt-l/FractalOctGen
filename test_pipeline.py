...
cond_out, logits = oct_ar(torch.randint(0, 2, (2, 64)).to(device),
                          torch.rand(2, 64, 3).to(device),
                          torch.rand(2, 1, 128).to(device),
                          torch.ones(2, 64, dtype=torch.bool).to(device))
assert cond_out.shape == (2, 64, 128) and logits.shape == (2, 64)
print('  [OK] OctAR：cond', tuple(cond_out.shape), 'logits', tuple(logits.shape))

# ── 5. OccupancyMLP（输出 logits + cond）──────────────────────────────────────
print('\n[5] 测试 OccupancyMLP 前向（级联输出）...')
mlp = OccupancyMLP(cond_dim=128, hidden_dim=128, num_layers=2).to(device)
lg, cd = mlp(torch.rand(2, 128, 3).to(device), torch.rand(2, 128, 128).to(device))
assert lg.shape == (2, 128) and cd.shape == (2, 128, 128)
print('  [OK] OccupancyMLP：logits', tuple(lg.shape), 'cond', tuple(cd.shape))
...
# ── 9. 体素转网格 ──────────────────────────────────────────────────────────────
print('\n[9] 测试体素转网格...')
from generate import sample_to_voxel_grid, voxel_to_mesh
voxel = sample_to_voxel_grid(so['split'][fd][0], so['xyz'][fd][0], finest_depth=fd)
print(f'  体素网格形状 {voxel.shape}，占据 {int(voxel.sum())}')
mesh = voxel_to_mesh(voxel.astype(float))
print(f'  网格顶点 {len(mesh.vertices)}，三角面 {len(mesh.faces)}')
print('  [OK] 体素转网格正确')
